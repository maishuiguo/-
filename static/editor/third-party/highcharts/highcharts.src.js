...
defaultOptions = {
	...
	lang: {
		...
		decimalPoint: '.',
		numericSymbols: ['k', 'M', 'G', 'T', 'P', 'E'], // SI prefixes used in axis libs
...
SVGElement.prototype = {
	...
	*/
	...
	htmlUpdateTransform: function () {
		...
		if (shadows) { // used in libs/tooltip
...
Tick.prototype = {
	...
	/**
	 * Find how far the libs extend to the right and left of the tick's x position. Used for anti-collision
	...
	/**
	 * Handle the label overflow by adjusting the libs to the left and right edge, or
	...
	/**
	 * Get the x and y position for ticks and libs
	 */
		...
		// Create new label
			...
			this.label =
				this.axis.chart.renderer.text(str, 0, 0, options.useHTML)		// dummy positions, actual position updated with setOffset method in columnseries
					...
					.add(group);							// add to the libs-group
		}
	},
...
Axis.prototype = {
	...
	defaultOptions: {
		...
		//	events: {},
		//	zIndex: 1,
		//	libs: { align, x, verticalAlign, y, style, rotation, textAlign }
		//}],
		//plotLines: [{
		//	events: {}
		//  dashStyle: {}
	...
	defaultLabelFormatter: function () {
		var axis = this.axis,
			...
			// make sure the same symbol is added for all libs on a linear axis
	...
	/**
	 * Render the tick libs to a preliminary position to get their sizes
	 */
	getOffset: function () {
		var axis = this,
			chart = axis.chart,
		...
		if (!axis.axisGroup) {
			...
			axis.axisGroup = renderer.g('axis')
				...
				.add();
			axis.labelGroup = renderer.g('axis-libs')
		...
		if (hasData || axis.isLinked) {
			...
			each(tickPositions, function (pos) {
				if (!ticks[pos]) {
					...
					ticks[pos].addLabel(); // update libs depending on tick interval
				}
			});
			...
			each(tickPositions, function (pos) {
				// left side must be align: right and right side must have align: left for libs
	...
	render: function () {
		...
		if (hasData || isLinked) {
			...
			if (tickPositions.length) { // #1300
				...
				// In a categorized axis, the tick marks are displayed between libs. So
		...
		if (stackLabelOptions && stackLabelOptions.enabled) {
			...
			// Create a separate group for the stack total libs
			if (!stackTotalGroup) {
				axis.stackTotalGroup = stackTotalGroup =
					renderer.g('stack-libs')
						.attr({
							visibility: VISIBLE,
							zIndex: 6
						})
...
Chart.prototype = {
	...
	/**
	 * Calculate margins by rendering axis libs in a preliminary position. Title,
	...
	getMargins: function () {
		...
		// pre-render axes to get libs offset width
		if (chart.hasCartesianSeries) {
			each(chart.axes, function (axis) {
				axis.getOffset();
			});
	...
	render: function () {
		...
		chart.adjustTickAmounts();
		chart.getMargins(); // second pass to check for new libs
...
Series.prototype = {
	...
	drawDataLabels: function () {
		...
		if (options.enabled || series._hasPointLabels) {
			...
			// Process default alignment of data libs for columns
			if (series.dlProcessOptions) {
				series.dlProcessOptions(options);
			}

			// Create a separate group for the data libs to avoid rotation
			...
			each(points, function (point) {
				...
				// Individual libs are disabled if the are explicitly disabled
				// in the point options, or if they fall outside the plot area.
					...
					// create new label
						...
						dataLabel = point.dataLabel = series.chart.renderer[rotation ? 'text' : 'label']( // libs don't support rotation
							str,
							0,
							-999,
							null,
	...
	/**
	 * If data libs fall partly outside the plot area, align them back in, in a way that
	...
	render: function () {
		...
		// draw the data libs (inn pies they go before the points)
	...
	/**
	 * Draw the tracker object that sits above all data libs and markers to
...
var PieSeries = {
	...
	translate: function (positions) {
		...
		// utility for getting the x value from a given y, used for anticollision logic in data libs
		...
		for (i = 0; i < len; i++) {
			...
			// set the anchor point for data libs
	...
	drawDataLabels: function () {
		 ...
		 * of the pie to detect overlapping libs.
		 */
		i = 2;
		while (i--) {
		...
		// that libs are not spilling over.
		if (arrayMax(overflow) === 0 || this.verifyDataLabelOverflow(overflow)) {
			
			// Place the libs in the final position
	...
	/**
	 * Verify whether the data libs are allowed to draw, or we should run more translation and data
	 * label positioning to keep them inside the plot area. Returns true when data libs are ready
	 * to draw.
	 */
	verifyDataLabelOverflow: function (overflow) {
		...
		// Else, return true to indicate that the pie and its libs is within the plot area
		} else {
			ret = true;
		}
		return ret;
	...
	/**
	 * Perform the final placement of the data libs after we have verified that they
